...
if __name__ == "__main__":
    ...
    if script_purpose == 'nomalize':
        ...
        target_path = 'full_sample_5'
        base_path = config['path']
        load_path = f'{base_path}/assets/receipt/{target_path}.png'
        
        receipt_image = receiptDetector.load_img(load_path)
        receipt_imutils_image = imutils.resize(receipt_image.co(py), width=len(receipt_image[0]))
        receipt_image_ratio = receipt_image.shape[1] / float(receipt_imutils_image.shape[1])
        
        # 이미지를 회색조로 변환합니다.
        receipt_imutils_gray_image = cv2.cvtColor(receipt_imutils_image, cv2.COLOR_BGR2GRAY)
        receipt_imutils_blurred_image = cv2.GaussianBlur(receipt_imutils_gray_image, (5, 5,), 0)
        receipt_imutils_edged_image = cv2.Canny(receipt_imutils_blurred_image, 75, 200)
        ...
        receipt = four_point_transform(receipt_image, receiptCnt.reshape(4, 2) * receipt_image_ratio)

        save_path = f'{base_path}/assets/receipt/{target_path}_output_1.png'
        receiptDetector.save_img(receipt_image, save_path)
        
        save_path = f'{base_path}/assets/receipt/{target_path}_output_2.png'
        receiptDetector.save_img(receipt_imutils_gray_image, save_path)
        
        save_path = f'{base_path}/assets/receipt/{target_path}_output_3.png'
        receiptDetector.save_img(receipt_imutils_blurred_image, save_path)
        
        save_path = f'{base_path}/assets/receipt/{target_path}_output_4.png'
        receiptDetector.save_img(receipt_imutils_edged_image, save_path)
        
        save_path = f'{base_path}/assets/receipt/{target_path}_output_5.png'
